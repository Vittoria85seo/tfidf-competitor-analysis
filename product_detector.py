...
_NAME_CLASS_PATTERN = re.compile(
    r"product.*(name|title)|name.*title|item.*(name|title)|card.*(name|title)|"
    r"product[-_]?label|item[-_]?label|prod[-_]?name", re.IGNORECASE)

_BROADER_NAME_PATTERN = re.compile(
    r"title|name|label|heading|brand", re.IGNORECASE)

_PRICE_LIKE = re.compile(r"^\s*\d+[.,]?\d*\s*(kr|:-|sek|eur|€|\$|£|%|off)\s*$", re.IGNORECASE)
...
def _get_name_from_element(el):
    ...
    product_name_selectors = [
        ("p", _NAME_CLASS_PATTERN),
        ("span", _NAME_CLASS_PATTERN),
        ("div", _NAME_CLASS_PATTERN),
        ("a", _NAME_CLASS_PATTERN),
    ]
    ...
    if link:
        ...
        if 8 <= len(aria) <= 200 and not _PRICE_LIKE.match(aria):
            return aria, "<a aria-label>", ""
        title_attr = (link.get("title") or "").strip()
        if 8 <= len(title_attr) <= 200 and not _PRICE_LIKE.match(title_attr):
            return title_attr, "<a title>", ""

    for tag in ("span", "p", "div", "a"):
        for child in el.find_all(tag, class_=True):
            cls = " ".join(child.get("class", []))
            if _BROADER_NAME_PATTERN.search(cls):
                txt = _clean_text(child.get_text(strip=True))
                if 8 <= len(txt) <= 200 and not _PRICE_LIKE.match(txt):
                    return txt, f"<{tag}>", cls
    ...
    if link:
        href = link.get("href", "")
        if href and "/" in href and len(href) > 10:
            link_text = _clean_text(link.get_text(strip=True))
            if 8 <= len(link_text) <= 200 and not _PRICE_LIKE.match(link_text):
                child_tags = [c.name for c in link.children if hasattr(c, "name") and c.name]
                if not any(t in child_tags for t in ("ul", "nav", "form")):
                    return link_text, "<a>", ""
...
def _extract_group_candidate(elements, tag_name, class_str, parent_name):
    ...
    uniqueness_ratio = len(unique_names) / len(products) if products else 1
    if len(unique_names) <= 2 and len(products) > 4:
        score -= 30
    elif uniqueness_ratio < 0.5:
        score -= 15

    if best_name_class:
        name_cls_lower = best_name_class.lower()
        if re.search(r"name|title", name_cls_lower):
            score += 10
...
def _find_product_groups(soup):
    ...
    if not candidates:
        _product_link_pattern = re.compile(
            r"/(?:product|produkt|p/|shop/|item/|vara/)", re.IGNORECASE)
        link_groups = Counter()
        link_elements = {}
        for a_tag in soup.find_all("a", href=True):
            if _is_nav_element(a_tag):
                continue
            href = a_tag.get("href", "")
            if not _product_link_pattern.search(href):
                continue
            parent = a_tag.parent
            if not parent or not parent.name:
                continue
            parent_classes = tuple(sorted(parent.get("class", [])))
            key = (parent.name, parent_classes)
            link_groups[key] += 1
            if key not in link_elements:
                link_elements[key] = []
            link_elements[key].append(parent)

        for key, count in link_groups.most_common(5):
            if count < 4:
                continue
            seen = set()
            unique_els = []
            for el in link_elements[key]:
                el_id = id(el)
            ...
            class_str = " ".join(key[1]) if key[1] else "(no class)"
            candidate = _extract_group_candidate(
                unique_els, tag_name, class_str, "product-link")
            if candidate:
                candidate["score"] += 8
                candidates.append(candidate)

    if not candidates:
        _data_attr_pattern = re.compile(r"product|item|sku|pid", re.IGNORECASE)
        data_groups = Counter()
        data_elements = {}
        for el in soup.find_all(["li", "article", "div", "section", "a"]):
            if _is_nav_element(el):
                continue
            for attr_name, attr_val in el.attrs.items():
                ...
                if _data_attr_pattern.search(attr_name):
                    key = (el.name, attr_name)
                    data_groups[key] += 1
                    if key not in data_elements:
                        data_elements[key] = []
                    data_elements[key].append(el)
                    break

        for key, count in data_groups.most_common(5):
            if count < 4:
                continue
            elements = data_elements[key]
            non_nav = [el for el in elements if not _is_nav_element(el)]
            if len(non_nav) < 4:
                continue
            tag_name = key[0]
            attr_name = key[1]
            candidate = _extract_group_candidate(
                non_nav, tag_name, attr_name, "data-attr")
            if candidate:
                candidate["score"] += 7
                candidates.append(candidate)
